...
class UR5:
    def __init__(self):
        ...
        self.multicall = xmlrpclib.MultiCall(self.proxy)
        # Start socket to ur5
        self.s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        self.s.connect(('192.168.1.20', 30003))
        time.sleep(0.05)

        self.target = []
        self.arm_max_linear_speed = 0.25
        self.arm_max_linear_accel = 1.2
        self.arm_max_joint_speed = 1.05
        self.arm_max_joint_accel = 1.4

        self.arm_default_linear_speed = 0.2
        self.arm_default_linear_accel = 0.8
        self.arm_default_joint_speed = 1.05
        self.arm_default_joint_accel = 1.4
    def dummy_stop(self):
        return False 
    ...
    def get_tcp_pose_vec(self):
        ...
        """
        # self.multicall.get_tcp_pose()
        self.target = self.proxy.get_tcp_pose()
        #print(self.target)
        return self.target
    ...
    def get_tcp_force(self):
        ...
        """
        return self.multicall.get_tcp_force()
    ...
    def get_joint_angles(self):
        ...
        """
        return self.multicall.get_joint_angles()  
    ...
    def wait_until_robot_is_finished(self, stop_condition='dummy'):
        ...
        while self.multicall.get_robot_status() == 3:
            if stop_condition():
                self.movel(self.multicall.get_tcp_pose())
                break
